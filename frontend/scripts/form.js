...
function fetchBasicDetails(){
    ...
    if(!basicDetails.firstName || !basicDetails.profession || !basicDetails.city || !basicDetails.state || !basicDetails.zipCode || !basicDetails.phoneNumber || !basicDetails.email){
...
function fetchPersonalDetails(){
    ...
    for(let i=0;i<personalDetailsInputs.length;i++){
        personalDetails[personalDetailsInputs[i].name] = (personalDetailsInputs[i].value)
    }

    //validation before dynamic div creation
    if(!personalDetails.gender || !personalDetails.maritalStatus || !personalDetails.fullAddress || !personalDetails.dob){
...
function fetchUnsavedData(event){
    ...
    // because if data is already extracted then the value of that field will be set to blank string
     ...
     let projectName = $(".projectValue");[0].value;
